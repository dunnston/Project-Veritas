...
func _on_building_placed(building_id: String, position: Vector3):
...
func refresh_building_structures():
	...
	for building in buildings_container.get_children():
		...
		var pos
		if building_system:
			pos = building_system.snap_to_grid_3d(building.global_position)
		else:
			pos = snap_to_grid_3d(building.global_position)
...
func identify_complete_buildings(floors: Array[Vector3], walls: Array[Vector3], doors: Array[Vector3], roofs: Array[Vector3]):
...
func find_rectangular_areas(positions: Array[Vector3]) -> Array[Array]:
...
func flood_fill_floors(start: Vector3, all_floors: Array[Vector3], processed: Array[Vector3]) -> Array[Vector3]:
	...
	while queue.size() > 0:
		...
		# Check 4 adjacent positions (using XZ plane for 3D, Y stays same)
		var adjacents = [
			pos + Vector3(32, 0, 0),   # Right (X+)
			pos + Vector3(-32, 0, 0),  # Left (X-)
			pos + Vector3(0, 0, 32),   # Forward (Z+)
			pos + Vector3(0, 0, -32)   # Back (Z-)
		]
...
func analyze_structure(floor_group: Array[Vector3], walls: Array[Vector3], doors: Array[Vector3], roofs: Array[Vector3]) -> Dictionary:
	"""Check if a floor group has complete walls, doors, and roofs around it"""

	# Find bounding box of floor group (using X and Z for horizontal plane)
	var min_x = floor_group[0].x
	var max_x = floor_group[0].x
	var min_z = floor_group[0].z
	var max_z = floor_group[0].z
	...
	var required_walls: Array[Vector3] = []
	var required_roofs: Array[Vector3] = []

	# Generate perimeter positions (walls should be on edges of floor tiles)
	for x in range(min_x, max_x + 32, 32):
		for z in range(min_z, max_z + 32, 32):
			var floor_pos = Vector3(x, floor_group[0].y, z)  # Keep same Y level
			if floor_pos in floor_group:
				...
				# Check each adjacent position for walls needed (32-unit grid)
				var adjacent_positions = [
					Vector3(x, floor_group[0].y, z - 32),     # Back adjacent (Z-)
					Vector3(x, floor_group[0].y, z + 32),     # Forward adjacent (Z+)
					Vector3(x - 32, floor_group[0].y, z),     # Left adjacent (X-)
					Vector3(x + 32, floor_group[0].y, z)      # Right adjacent (X+)
				]
...
func floor_at_position(pos: Vector3, floor_group: Array[Vector3]) -> bool:
	"""Check if there's a floor tile at the given position"""
	return pos in floor_group

func count_coverage(required: Array[Vector3], available: Array[Vector3]) -> int:
...
func _process(_delta):
	...
	var player_pos
	if building_system:
		player_pos = building_system.snap_to_grid_3d(player.global_position)
	else:
		player_pos = snap_to_grid_3d(player.global_position)
...
func find_building_at_position(pos: Vector3) -> String:
...
func snap_to_grid_3d(pos: Vector3, grid_size: int = 32) -> Vector3:
	"""Snap 3D position to grid - converts to 3D grid system"""
	return Vector3(
		round(pos.x / grid_size) * grid_size,
		pos.y,  # Keep Y as-is for vertical position
